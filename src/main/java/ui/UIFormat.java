package ui;

import java.awt.*;

/**
 * Stores all background color, Font, and text color info needed by the UI
 */
public class UIFormat {

    // BACKGROUND COLORS
    public static final Color SIDE_PANEL_BACKGROUND = new Color(247, 238, 233);
    public static final Color MAIN_PANEL_BACKGROUND = new Color(247, 237, 226);
    public static final Color YEllOW = new Color(246, 189, 96);
    public static final Color TASK_SCROLL = new Color(251, 223, 178);
    public static final Color BUTTON_PINK = new Color(240, 177, 168);
    public static final Color HEAL_BUTTON = new Color(242, 132, 130);
    public static final Color LIGHT_GREEN = new Color(167, 187, 182);
    public static final Color GREEN = new Color(132, 165, 157);

    // FONTS
    public static final Font TITLE_FONT = new Font("Krungthep", Font.PLAIN, 36);
    public static final Font BUTTON_FONT = new Font("Krungthep", Font.PLAIN, 13);
    public static final Font POINTS_FONT = new Font("Krungthep", Font.PLAIN, 18);
    public static final Font PET_NAME_FONT = new Font("Krungthep", Font.PLAIN, 20);

    // TEXT COLORS
    public static final Color GREEN_TEXT = new Color(97, 152, 142);
    public static final Color PINK_TEXT = new Color(242, 132, 130);
    public static final Color WHITE_TEXT = Color.WHITE;

}
